...
use certs::{CertPaths};
use fetch::{ProcType,Endorsement,cert_authority::fetch_ca,vcek::fetch_vcek};
use serde_json;
...
pub async fn cmd(att_report_path: AttestationReport, certificate:Vec<u8>, miner_ip:String, measurement: String, quiet: bool) -> Result<()> {
    let vek = match attestation::validate_cc(Path::new("./milan_certs"), ProcType::Milan, att_report_path.clone(), quiet).await {
        Ok(vek) => Ok(vek),
        Err(_error) => match attestation::validate_cc(Path::new("./genoa_certs"), ProcType::Genoa, att_report_path.clone(), quiet).await {
            Ok(vek) => Ok(vek),
            Err(error) => Err(error),
        }
    }?;
    
    attestation::verify_attestation(att_report_path.clone(), vek, Path::new("."), quiet)?;

    let report_measurement = hex::encode(&att_report_path.measurement);
    if measurement == report_measurement {
        //Hash the certificate to compare it with the hash in the attestation report
        let cert_hash = get_cert_hash(certificate.clone())?;

        if cert_hash == att_report_path.report_data {
            let row = Miner{cert: std::str::from_utf8(&certificate)?.to_string(), ip: miner_ip};

            let db_file = OpenOptions::new().create(true).append(true).open("db.json")?;
            serde_json::to_writer(db_file, &row)?;
            return Ok(())
        }  
    }
    Err(anyhow::anyhow!("Failed to verify attestation report"))
}


fn get_cert_hash(certificate: Vec<u8>) -> Result<[u8; 64], anyhow::Error>
{
    let mut hash_cert = Sha256::new();
    hash_cert.update(certificate);
    let hash_certificate = hash_cert.finalize();

    let hash_array : [u8; 32] = hash_certificate.as_slice().try_into().expect("Failed to hash certificate");
    let zero_array : [u8; 32] = [0; 32];
    let data_report: [u8; 64] = [&zero_array[..], &hash_array].concat().try_into().expect("Failed to hash certificate");

    return Ok(data_report)
}
...
pub fn find_cert_in_dir(dir: &Path, cert: &str) -> Result<PathBuf, anyhow::Error> {
    if dir.join(format!("{cert}.pem")).exists() {
        ...
        Err(anyhow::anyhow!("{cert} certificate not found in directory or retrievable from KDS"))
    }
}
...
pub mod attestation {
    ...
    pub async fn validate_cc(certs_dir:&Path, proc_type:ProcType, att_report_path: AttestationReport, quiet: bool) -> Result<Certificate> {
        let ark_path = match find_cert_in_dir(&certs_dir, "ark") {
            Ok(ark_path) => ark_path,
            Err(_) => {
                //Fetch CA certs for requested processor type
                fetch_ca(proc_type.clone(), Endorsement::Vcek, certs_dir.to_path_buf()).await?; //Path::new("./milan_root"));
                find_cert_in_dir(certs_dir, "ark")?
            } 
        };

        let ask_path = find_cert_in_dir(&certs_dir, "ask")?;
        let vek = fetch_vcek(proc_type, att_report_path, Path::new(".")).await?;
        
        let vek_type = "vcek";
        let sign_type = "ask";
    
        // Get a cert chain from directory
        let cert_chain: ca::Chain = CertPaths {
            ark_path,
            ask_path,
        }
        .try_into()?;
    
        let ark = cert_chain.ark;
        let ask = cert_chain.ask;
    ...
    pub fn verify_attestation(att_report: AttestationReport, vek: Certificate, _vcek_path: &Path, quiet: bool) -> Result<()> {
